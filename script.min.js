...
async function typeBio() {
    ...
    for (let i = 0; i < bioText.length; i++) {
        ...
        if (i > 0) {
            const prevLine = bioElement.children[i - 1];
...
function updateTerminal() {
    ...
    if (storedIpData && storedGeoData) {
        const proxyWarning = storedGeoData.proxy ? 
            '<span class="gruv-yellow"> (Possible VPN/Proxy detected, location may be inaccurate)</span>' : '';
        terminal.innerHTML = `
            <div class="terminal-line">
                <span class="prompt">$</span> ${greeting}, Visitor detected!
            </div>
            <div class="terminal-line">
                <span class="prompt">$</span> Visitor's IP: <span class="gruv-yellow">${storedIpData.ip}</span>
            </div>
            <div class="terminal-line">
                <span class="prompt">$</span> Location (approximate): ${storedGeoData.city}, ${storedGeoData.region_name}, ${storedGeoData.country_name}${proxyWarning}
            </div>
            <div class="terminal-line">
                <span class="prompt">$</span> Status: <span class="gruv-green">Connection secure</span>
            </div>
        `;
    }
}

// Visitor Detection with ipstack
async function detectVisitorInfo() {
    ...
    try {
        ...
        const apiKey = '1b2488ef52871a211f18ee54bd448c42'; // 
        const geoResponse = await fetch(`http://api.ipstack.com/${storedIpData.ip}?access_key=${apiKey}`);
        storedGeoData = await geoResponse.json();

        // Check for errors in ipstack response
        if (storedGeoData.error) {
            throw new Error(storedGeoData.error.info);
        }
        ...
        terminal.innerHTML = `
            ...
            <div class="terminal-line">
                <span class="prompt">$</span> Status: <span class="gruv-yellow">Possible VPN/Proxy or API error</span>
            </div>
        `;
    }
}
...
const cliCommands = {
    ...
    `,
    about: () => `
        Christian Isaac .S (0xb0rn3 | 0xbv1)<br>
    ...
    timeline: () => `
        ...
        - 2020: Started learning cybersecurity<br>
        - 2021: Completed first CTF challenge<br>
        - 2022: Earned Google Cybersecurity Certificate<br>
        - 2023: Began working as a Jr. Penetration Tester
    `,
    clear: () => {
